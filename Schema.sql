...
CREATE TABLE IF NOT EXISTS `biometric_project1`.`Transaction` (
  ...
  `amount` DECIMAL(15) NOT NULL COMMENT '',
  `date` DATETIME(6) NOT NULL COMMENT '',
...
CREATE TABLE IF NOT EXISTS `biometric_project1`.`TransactionType` (
  `customerTransactionId` INT NOT NULL COMMENT '',
  `transactionType` VARCHAR(45) NOT NULL COMMENT '',
  `customerTransactionAccountId` INT NOT NULL COMMENT '',
  CONSTRAINT `customerTransactionId`
    FOREIGN KEY (`customerTransactionId`)
    REFERENCES `biometric_project1`.`Transaction` (`TransactionId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `AccountIdcustomerTransactionAccountId`
    FOREIGN KEY (`customerTransactionAccountId`)
    REFERENCES `biometric_project1`.`Account` (`AccountId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
